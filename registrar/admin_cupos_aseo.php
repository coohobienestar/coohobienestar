...
<!--
   ...
   function actualizaPagina (){
      ...
      window.location = 'admin_cupos_aseo.php?departamento='+departamento+'&pagina='+pagina;
   }
// -->
</SCRIPT>
...
session_start();
  ...
  error_consulta($result,$registro);                           
    ...
    if(($cod_usuario != '') && ($autorizado==1)){
      ...
      print ("<FORM NAME='datechooser' ACTION='admin_cupos_aseo.php' METHOD='POST'>");
 ...
 if($cod_departamento != ''){  
    ...
    ////BORRAMOS LA TABLA DE CUPOS DE ASEO 
    $instruccion_del = "DELETE FROM 0as_escuela";
    $consulta_del = mysql_query ($instruccion_del, $conexion);      
         
      ////EJECUTAMOS LA CONSULTA
      $instruccion2 ="SELECT escuela.cod_escuela AS cod_escuela, escuela.nombre AS nom_escuela, municipio.nombre AS nom_municipio, escuela.cod_municipio AS cod_municipio, 
                             tipo_minuta.cod_tipo_minuta AS cod_tipo_minuta, tipo_minuta.nombre AS nom_tipo_minuta,
                             centro_acopio.nombre AS nom_centro_acopio, escuela.cod_centro_acopio AS cod_centro_acopio
                      FROM minuta_escuela 
      ...
      if ($nfilas > 0){
        ...
        $color = '';
         ...
         for ($i=0; $i<$nfilas; $i++){
            ...
            ////INSERTAMOS EN LA TABLA TEMPORAL DE ESCUELAS PARA EL ASEO
            $instruccion4 = "INSERT INTO 0as_escuela (cod_escuela, cod_tipo_minuta, nombre, cod_centro_acopio, cod_municipio, cod_departamento, total_cupos) 
                             VALUES ('$row2[cod_escuela]',$row2[cod_tipo_minuta],'$row2[nom_escuela]','$row2[cod_centro_acopio]','$row2[cod_municipio]','$cod_departamento','$cupos_total')";
         ...
         echo $hojaExcel;  
          ...
          $fecha = date("Ymd_His");
          $sfile="../excel/InformeCupos_aseo"._."$login"._."$fecha.xls"; //ruta del archivo a generar 
          $fp=fopen($sfile,"w"); 
          fwrite($fp,$hojaExcel); 
          fclose($fp);
