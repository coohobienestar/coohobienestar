...
<!--
   ////FUNCION PARA HACER OPERACIONES SOBRE LAS TABLAS [REGISTRAR (1) - EDITAR (2)]
    ...
    var url="../admin/operacion_departamento.php?codigo="+codigo+"&tipo_operacion="+tipo_operacion;
   ...
   function actualizaPagina (){
      ...
      l = document.forms.datechooser.operador.selectedIndex;
      operador = document.forms.datechooser.operador.options[l].value;
            
      k = document.forms.datechooser.pagina.selectedIndex;
      pagina = document.forms.datechooser.pagina.options[k].value;
            
      window.location = 'admin_departamento.php?codigo='+codigo+'&nombre='+nombre+'&pagina='+pagina+'&operador='+operador;
   }
// -->
</SCRIPT>
...
<?php if($opcion_vista == 1){?>
 <td width='30%' style='font-weight:bold; color: #f4d359' align="center"><a href=javascript:operar_tabla(0,1)><img src='../imagenes/guardar.png' width='32' height='32' border='0' alt='Nuevo Registro'>&nbsp;&nbsp;Registrar Departamento</a></td>
<?php } ?>
...
<?php                        
      ...
      print ("<FORM NAME='datechooser' ACTION='admin_departamento.php' METHOD='POST'>");
      ...
      if($codigo != ''){
         $condicion2 = $condicion2. "departamento.cod_departamento like '%$codigo%' AND ";
        }
      if($nombre != ''){
         $condicion2 = $condicion2. "departamento.nombre like '%$nombre%' AND ";
        } 
      if($operador != ''){
         $condicion2 = $condicion2. "departamento.cod_operador = '$operador' AND ";
        }         
      ...
      $instruccion2 ="SELECT departamento.cod_departamento AS cod_departamento, departamento.nombre AS nombre, departamento.cod_operador AS cod_operador,
                             operador.nombre AS nom_operador
                      FROM departamento
                      INNER JOIN operador ON operador.cod_operador = departamento.cod_operador
                      $condicion_final 
      ...
      $hojaExcel.="<TR><TH colspan='9'><center>DEPARTAMENTOS</center></TH></TR>";       
      $hojaExcel.="<TH><center>Código</center></TH>";
      $hojaExcel.="<TH><center>Nombre</center></TH>";
      $hojaExcel.="<TH colspan='2'><center>Operador</center></TH>";
    if($opcion_vista == 1){
      ...
      $color = '';
         ...
         for ($i=0; $i<$nfilas; $i++){
            ...
            $hojaExcel.="<TD style=background:$color>" . $row2['cod_operador'] . "</TD>";
            $hojaExcel.="<TD style=background:$color>" . $row2['nom_operador'] . "</TD>";
          if($opcion_vista == 1){   
            $hojaExcel.="<TD style=background:$color><center> <a href=javascript:operar_tabla($row2[cod_departamento],2) title='Editar Ciclo'><img src='../imagenes/editar.png' width='14' height='14' border='0' alt='Editar'></a> </center></TD>"; 
            } 
            $hojaExcel.="</TR>";
         }
         $hojaExcel.="</TABLE>";
