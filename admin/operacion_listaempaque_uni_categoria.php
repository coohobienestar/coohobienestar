...
$cod_categoria_sel = $_GET['categoria'];
$ingrediente = $_GET['ingrediente'];
$municipio = $_GET['municipio'];


////SI SE DESEA CONSULTAR UN UNICO INGREDIENTE SE AGREGA ESTA INSTRUCCION A LA CONSULTA
if($ingrediente != '0'){
   $condicion_ing = " AND calculo_redondeado_escuela.cod_ingrediente = $ingrediente ";
  }else{
    $condicion_ing = " "; 
    }
    
////SI SE DESEA CONSULTAR UN UNICO MUNICIPIO SE AGREGA ESTA INSTRUCCION A LA CONSULTA
if($municipio != '0'){
   $condicion_mun = " AND calculo_requerimientos.cod_municipio = $municipio ";
  }else{
    $condicion_mun = " "; 
    }    
...
$cod_tipo_minuta = $row_0['cod_tipo_minuta'];
  ...
  $instruccion2 ="SELECT DISTINCT calculo_requerimientos.cod_municipio AS cod_municipio, municipio.nombre AS nom_municipio, 
                  ...
                  WHERE calculo_requerimientos.cod_programacion = $cod_programacion AND calculo_requerimientos.cod_departamento = $cod_departamento 
                    AND calculo_requerimientos.cod_tipo_minuta = $cod_tipo_minuta $condicion_mun  
  ...
  for ($j=0; $j<$nfilas2; $j++){
    ...
    $nom_tipo_minuta = $row2['nom_tipo_minuta'];
      ...
      ////BUSCAMOS LAS CATEGORIAS DE LOS ALIMENTOS SEGUN SE ENVIA POR EL FORMULARIO
      $instruccion ="SELECT cod_categoria_ingrediente, nombre FROM categoria_ingrediente 
                     WHERE cod_categoria_ingrediente = $cod_categoria_sel 
                     ORDER BY cod_categoria_ingrediente";
      ...
      for ($i=0; $i<$nfilas; $i++){
        ...
        $instruccion3 ="SELECT DISTINCT calculo_redondeado_escuela.cod_escuela AS cod_escuela, escuela.nombre AS nom_escuela 
                        ...
                        WHERE calculo_redondeado_escuela.cod_programacion = $cod_programacion AND calculo_redondeado_escuela.cod_municipio = $cod_municipio  
                          ...
                          AND calculo_redondeado_escuela.cod_tipo_minuta = $cod_tipo_minuta $condicion_ing
...
<html>
<head>
<title>INFORME LISTA DE EMPAQUE POR CATEGORIAS</title>
</head>
<body>  
  <?php 
    echo ($hojaExcel);
    ...
    $sfile="../excel/ope_lista_empaque_categoria"._."$login"._."$fecha.xls"; //ruta del archivo a generar 
    $fp=fopen($sfile,"w"); 
    fwrite($fp,$hojaExcel); 
    fclose($fp);
