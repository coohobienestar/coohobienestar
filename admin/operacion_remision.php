...
$hojaExcel.="<table width='98%' border='1'>";
  ...
  $hojaExcel.="</tr>";  
      ...
      for ($i=0; $i<$nfilas7; $i++){
        ...
        ////BUSCAMOS LAS FORMAS DE EMPAQUE
        $instruccion_for_emp ="SELECT unidad_empaque.nombre AS nom_unidad_empaque, unidad_empaque.nombre_simple AS nom_simple, 
                                      unidad_empaque.unidades_empaque AS unidades_empaque, unidad_empaque.cod_unidad_empaque AS cod_unidad_empaque
                               FROM ingrediente_unidad_entrega 
                               INNER JOIN unidad_empaque ON unidad_empaque.cod_unidad_empaque = ingrediente_unidad_entrega.cod_unidad_empaque
                               WHERE ingrediente_unidad_entrega.cod_ingrediente = $cod_ingrediente 
                               AND ingrediente_unidad_entrega.cod_unidad_medida = $cod_unidad_medida";
     
        $consulta_for_emp = mysql_query($instruccion_for_emp);
        error_consulta($consulta_for_emp,$instruccion_for_emp);
        $row_for_emp = mysql_fetch_array($consulta_for_emp);  
        
        $cod_unidad_empaque = $row_for_emp['cod_unidad_empaque']; 
        $nom_unidad_empaque = $row_for_emp['nom_unidad_empaque'];        
        $nom_simple = $row_for_emp['nom_simple']; 
        $unidades_empaque = $row_for_emp['unidades_empaque'];
        
        $cad_empaque = ""; 
        $cad_empaque2 = "";
        $cad_final = "";
        
        if($cod_unidad_empaque != 0 || $cod_unidad_empaque != ''){
        
         ////calculamos la cantidad en la unidad de empaque
         $cantidad_ent_uni_emp = floor($cantidad / $unidades_empaque); 
          
         $cad_empaque = "$cantidad_ent_uni_emp $nom_simple";   
        
         $cant_unidades = $cantidad - ($cantidad_ent_uni_emp * $unidades_empaque);
         
         if($cant_unidades == 1){
            $cad_empaque2 = " $cant_unidades Unidad";
           }else{
              $cad_empaque2 = " $cant_unidades Unidades";
             }                 
               
         $conector = " Y ";
         
         $cad_final = $cad_empaque.$conector.$cad_empaque2; 
        ...
        $hojaExcel.="<td>$nom_ingrediente</td>";
        $hojaExcel.="<td align='center'>$nom_unidad</td>";
        $hojaExcel.="<td align='center'>$nom_unidad_empaque</td>";
        $hojaExcel.="<td align='center'>$cad_final</td>";
        $hojaExcel.="<td align='center'>$cantidad</td>";
        $hojaExcel.="</tr>"; 
       } 
