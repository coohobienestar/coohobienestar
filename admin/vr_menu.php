...
if($tipo_operacion <= 2){
  ...
  if (empty($_REQUEST['nombre']))
    $error=$error . "El Nombre del Menu no puede estar vacio.<br>";   
...
$nombre    = strtoupper($_REQUEST[nombre]);
  ...
  if($tipo_operacion == 1){
     mysql_query("INSERT INTO menu (nombre) VALUES ('$nombre')", $conexion);
    }  

  if($tipo_operacion == 2){
     $instruccion4 = "UPDATE menu SET nombre='$nombre' WHERE cod_menu = '$codigo'";
