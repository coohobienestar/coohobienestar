...
if($nfilas_usu > 0){
 ...
 if($cod_municipio == 0 && $cod_escuela == 0 && $tipo != 0){
  ...
  $instruccion2 ="SELECT DISTINCT calculo_requerimientos_inf.cod_ciclo AS cod_ciclo, ciclo.nombre AS nom_ciclo, calculo_requerimientos_inf.cod_escuela AS cod_escuela, 
                  ...
                  INNER JOIN departamento ON departamento.cod_departamento = calculo_requerimientos_inf.cod_departamento
                  INNER JOIN municipio ON municipio.cod_municipio = calculo_requerimientos_inf.cod_municipio
                  INNER JOIN centro_acopio ON centro_acopio.cod_centro_acopio = calculo_requerimientos_inf.cod_centro_acopio
  ...
  $instruccion_re ="SELECT DISTINCT calculo_requerimientos_inf.cod_rango_edad AS cod_rango_edad, rango_edad.nombre AS nombre
                    FROM calculo_requerimientos_inf 
                    INNER JOIN rango_edad ON rango_edad.cod_rango_edad = calculo_requerimientos_inf.cod_rango_edad  
                    WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad
                      AND calculo_requerimientos_inf.cod_municipio = $cod_municipio AND calculo_requerimientos_inf.cod_escuela = $cod_escuela 
                      AND calculo_requerimientos_inf.cod_tipo_minuta = $cod_tipo_minuta";
...
$hojaExcel.="</table>";   
  ...
  $jornada = $row_jor['cod_jornada'];  
      ...
      $instruccion_men ="SELECT DISTINCT calculo_requerimientos_inf.cod_menu AS cod_menu
                         FROM calculo_requerimientos_inf 
                         INNER JOIN rango_edad ON rango_edad.cod_rango_edad = calculo_requerimientos_inf.cod_rango_edad  
                         WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad
                         AND calculo_requerimientos_inf.cod_municipio = $cod_municipio AND calculo_requerimientos_inf.cod_escuela = $cod_escuela
      ...
      $instruccion6 ="SELECT DISTINCT calculo_requerimientos_inf.cod_rango_edad AS cod_rango_edad, rango_edad.nombre AS nombre, 
                                      calculo_requerimientos_inf.cupos AS cupos
                      FROM calculo_requerimientos_inf 
                      INNER JOIN rango_edad ON rango_edad.cod_rango_edad = calculo_requerimientos_inf.cod_rango_edad  
                      WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad
                        AND calculo_requerimientos_inf.cod_municipio = $cod_municipio AND calculo_requerimientos_inf.cod_escuela = $cod_escuela
      ...
      $instruccion_ct ="SELECT DISTINCT calculo_requerimientos_inf.cod_rango_edad AS cod_rango_edad, rango_edad.nombre AS nombre, 
                                      calculo_requerimientos_inf.cupos AS cupos
                      FROM calculo_requerimientos_inf 
                      INNER JOIN rango_edad ON rango_edad.cod_rango_edad = calculo_requerimientos_inf.cod_rango_edad  
                      WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad
...
$hojaExcel.="<table width='98%' border='1'>";
  ...
  $hojaExcel.="</tr>";
      ...
      $instruccion7 ="SELECT calculo_requerimientos_inf.cod_ingrediente AS cod_ingrediente, ingrediente.nombre AS nom_ingrediente, 
                      calculo_requerimientos_inf.cod_categoria_ingrediente AS cod_categoria, categoria_ingrediente.nombre AS nom_categoria, 
                      SUM(calculo_requerimientos_inf.cantidad) AS cantidad, ingrediente.unidad_base AS unidad_base 
                      FROM calculo_requerimientos_inf 
                      ...
                      WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_escuela = $cod_escuela 
                        AND calculo_requerimientos_inf.cod_municipio = $cod_municipio AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad
                        AND calculo_requerimientos_inf.cod_tipo_minuta = $cod_tipo_minuta
                      GROUP BY calculo_requerimientos_inf.cod_ingrediente
      ...
      $hojaExcel.="<tr>";
       ...
       ////BUSCAMOS LOS DEMAS DATOS DE LOS INGREDIENTES
        $instruccion8 ="SELECT calculo_redondeado_escuela_inf.cod_unidad_medida AS cod_unidad_medida, unidad_medida.nombre AS nom_unidad, 
                               calculo_redondeado_escuela_inf.cantidad_gr_cc AS cantidad_gr_cc, calculo_redondeado_escuela_inf.cantida_bruta AS cantida_bruta, 
                               calculo_redondeado_escuela_inf.cantidad_redondeada AS cantidad_redondeada
                        ...
                        WHERE calculo_redondeado_escuela_inf.cod_programacion = $cod_programacion AND calculo_redondeado_escuela_inf.cod_municipio = $cod_municipio
                          AND calculo_redondeado_escuela_inf.cod_escuela = $cod_escuela AND calculo_redondeado_escuela_inf.cod_ingrediente = $cod_ingrediente 
...
if ($cod_tipo_minuta == 3 || $cod_tipo_minuta == 16 || $cod_tipo_minuta == 19 || $cod_tipo_minuta == 23 || $cod_tipo_minuta == 17 || $cod_tipo_minuta == 20 ||
    $cod_tipo_minuta == 57 || $cod_tipo_minuta == 58 || $cod_tipo_minuta == 81 || $cod_tipo_minuta == 82){

 if($cod_tipo_minuta == 17 || $cod_tipo_minuta == 20 || $cod_tipo_minuta == 58 || $cod_tipo_minuta == 81 || $cod_tipo_minuta == 82){
   $titulo ="REMISIÓN ENTREGA DE VÍVERES EN COMEDORES ESCOLARES - RACIÓN PREPARADA EN SITIO - COMPLEMENTO ALIMENTARIO JM/JT ";
   }

 if($cod_tipo_minuta == 3 || $cod_tipo_minuta == 16 || $cod_tipo_minuta == 19 || $cod_tipo_minuta == 23 || $cod_tipo_minuta == 57){
 ...
 $hojaExcel.="<tr>";
      ...
      for ($i=0; $i<$nfilas7; $i++){
        ...
        for ($h=0; $h<$nfilas6; $h++){             
          ...
          $instruccion_q ="SELECT (SUM(calculo_requerimientos_inf.cantidad) / calculo_requerimientos_inf.cupos) AS racion 
                           FROM calculo_requerimientos_inf 
                           WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion  AND calculo_requerimientos_inf.cod_municipio = $cod_municipio
                             AND calculo_requerimientos_inf.cod_escuela = $cod_escuela 
                             AND calculo_requerimientos_inf.cod_rango_edad = $cod_rango AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad 
...
echo $hojaExcel; 
    ...
    $sfile="../excel/FormatoListaEntregaPorMunic"._."$login"._."$fecha.xls"; //ruta del archivo a generar 
    $fp=fopen($sfile,"w"); 
    fwrite($fp,$hojaExcel); 
    fclose($fp);
