...
$tipo_operacion = $_REQUEST[tipo_operacion];
 ...
 if($tipo_operacion == 1){
  ...
  if($_REQUEST['producto'] == '')   
   ...
   $instruccion3 = "SELECT 0as_paquete_producto.cod_producto AS cod_producto
                    FROM 0as_paquete_producto 
                    WHERE 0as_paquete_producto.cod_paquete = $_REQUEST[paquete0] AND 0as_paquete_producto.cod_producto = $_REQUEST[producto]";
  ...
  if(filter_var($_REQUEST['cantidad'], FILTER_VALIDATE_INT) === false)  
    $error=$error . "La Cantidad debe ser un Valor Entero.<br>";

  if($_REQUEST['cantidad'] <=0)   
    $error=$error . "La Cantidad debe ser Mayor que Cero<br>"; 
    
  if(filter_var($_REQUEST['relacion'], FILTER_VALIDATE_INT) === false)  
    $error=$error . "La Relacion (Por cada) debe ser un Valor Entero.<br>";

  if($_REQUEST['relacion'] <0)   
    $error=$error . "La Relacion (Por cada) debe ser Mayor o igual a Cero<br>"; 
