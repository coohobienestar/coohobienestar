...
if($tipo_operacion == 1){
  ...
  if (empty($_REQUEST['nombre']))
    $error=$error . "El Nombre del Operador no puede estar vacio.<br>";   

  if (empty($_REQUEST['nit']))
    $error=$error . "El Nit del Operador no puede estar vacio.<br>";    

  if (empty($_REQUEST['logo']))
    $error=$error . "El Logo del Operador no puede estar vacio.<br>";
...
$logo = $_REQUEST[logo];
  ...
  if($tipo_operacion == 1){
     mysql_query("INSERT INTO operador (nombre, nit, logo) VALUES ('$nombre','$nit','$logo')", $conexion);
    }  

  if($tipo_operacion == 2){
     $instruccion4 = "UPDATE operador SET nombre='$nombre', nit='$nit'  WHERE cod_operador = '$codigo'";
