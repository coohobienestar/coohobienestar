...
if($tipo_operacion == 2){
   ...
   $instruccion3 = "SELECT email, cedula, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido FROM 0_bdcrio WHERE id = $codigo";
   $consulta3 = mysql_query ($instruccion3, $conexion);  
   $row3 = mysql_fetch_array ($consulta3);

   $email = $row3['email'];
   $cedula = $row3['cedula']; 
   $p_nombre = $row3['primer_nombre']; 
   $s_nombre = $row3['segundo_nombre']; 
   $p_apellido = $row3['primer_apellido'];
   $s_apellido = $row3['segundo_apellido']; 
   
   $nombre_c =  $p_nombre." ".$s_nombre." ".$p_apellido." ".$s_apellido;
...
<body>
<form action="vr_crio.php" method="post">
...
?> 
  <tr>
   <td style='font-weight:bold; color: white'>ID</td>
   <td><input type="text" Disabled name="codigo" size="5" value='<?php print("$codigo");?>'></td>   
  </tr>
  <tr>
   <td style='font-weight:bold; color: white'>Cedula</td>
   <td style='font-weight:bold; color: white'><?php print("$cedula");?></td>   
  </tr>
  <tr>
   <td style='font-weight:bold; color: white'>Nombre</td>
   <td style='font-weight:bold; color: white'><?php print("$nombre_c");?></td>   
  </tr>  
  <tr>
   <td style='font-weight:bold; color: white'>Email</td>
   <td><img src="../imagenes/requerido.gif"><input type="text" name="email" size="50" value='<?php print("$email");?>'></td>   
  </tr>     
  <tr>
