...
$clasificacion = $_REQUEST[clasificacion];
  ...
  if($tipo_operacion == 1){
     mysql_query("INSERT INTO 0c_documento_registro (cod_usuario,nombre_documento, ult_fecha_revision, cod_clasificacion, obsoleto) 
         VALUES ('$usuario','$nombre','$fecha_ult','$clasificacion','0')", $conexion);
    }  

  if($tipo_operacion == 2){
      $cod_documento = $_REQUEST[documento0];
      
      $instruccion3 = "UPDATE 0c_documento_registro SET nombre_documento = '$nombre', ult_fecha_revision = '$fecha_ult', cod_clasificacion = '$clasificacion'
  ...
  if($tipo_operacion == 3){
      ...
      for($f=0;$f<$nfilas;$f++){
       ...
       if($v_cod_usuario!=''){
         ...
         $instruccion_del = "DELETE FROM 0c_usuario_documento_registro WHERE cod_usuario = $cod_usuario AND cod_documento = $cod_documento"; 
         $consulta_del = mysql_query ($instruccion_del, $conexion); 
           
         ////INSERTAMOS LOS DOCUMENTOS DEL USUARIO
         $instruccion5 = "INSERT INTO 0c_usuario_documento_registro (cod_usuario, cod_documento) values ('$cod_usuario','$cod_documento')"; 
  ...
  if($tipo_operacion == 4){
    ...
    $instruccion3 = "UPDATE 0c_documento_registro SET obsoleto = '1' WHERE cod_documento = '$cod_documento'";
    $consulta3 = mysql_query ($instruccion3, $conexion); 
   }     
