...
$sql_1a = "SELECT DISTINCT cod_programacion_inventario FROM calculo_redondeado_escuela_inf WHERE cod_programacion=$cod_programacion";
...
if($nfilas_usu > 0){
 ...
 if($cod_escuela != 0){
  $condicion = " WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_escuela = $cod_escuela 
                   AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad AND calculo_requerimientos_inf.cod_tipo_minuta = $tipo";
  }
  
 if($cod_municipio != 0){
  $condicion = " WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_municipio = $cod_municipio 
                   AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad AND calculo_requerimientos_inf.cod_tipo_minuta = $tipo";
  }  

 if($cod_municipio == 0 && $cod_escuela == 0 && $tipo != 0){
  $condicion = " WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_tipo_minuta = $tipo";
  }    

  /////CONSULTA PARA HACER EL INFORME DE TODOS LOS REGISTROS
  $instruccion ="SELECT DISTINCT calculo_requerimientos_inf.cod_escuela AS cod_escuela, escuela.nombre AS nom_escuela, 
                                 ...
                                 calculo_requerimientos_inf.cod_tipo_minuta AS cod_tipo_minuta, tipo_minuta.nombre AS nom_tipo_minuta
                  ...
                  $condicion
                  ORDER BY tipo_minuta.nombre, calculo_requerimientos_inf.cod_departamento, calculo_requerimientos_inf.cod_municipio, calculo_requerimientos_inf.cod_escuela                       
                  ";
  ...
  $instruccion2 ="SELECT DISTINCT calculo_requerimientos_inf.cod_ciclo AS cod_ciclo, ciclo.nombre AS nom_ciclo, calculo_requerimientos_inf.cod_escuela AS cod_escuela, 
                                  escuela.nombre AS nom_escuela, calculo_requerimientos_inf.cod_departamento AS cod_departamento, 
                                  departamento.nombre AS nom_departamento, calculo_requerimientos_inf.cod_municipio, municipio.nombre AS nom_municipio
                  FROM calculo_requerimientos_inf
                  INNER JOIN ciclo ON ciclo.cod_ciclo = calculo_requerimientos_inf.cod_ciclo
                  INNER JOIN escuela ON escuela.cod_escuela = calculo_requerimientos_inf.cod_escuela
                  INNER JOIN departamento ON departamento.cod_departamento = calculo_requerimientos_inf.cod_departamento
                  INNER JOIN municipio ON municipio.cod_municipio = calculo_requerimientos_inf.cod_municipio
                  WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_municipio = $cod_municipio 
                    AND calculo_requerimientos_inf.cod_escuela = $cod_escuela AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad 
                    AND calculo_requerimientos_inf.cod_tipo_minuta = $cod_tipo_minuta
                  ORDER BY calculo_requerimientos_inf.cod_departamento, calculo_requerimientos_inf.cod_municipio, calculo_requerimientos_inf.cod_escuela                       
                  ";
 ...
 ////DEFINIMOS EL TIPO DE MINUTA DE DESAYUNOS INDUSTRIALIZADOS
if ($cod_tipo_minuta == 4 || $cod_tipo_minuta == 50 || $cod_tipo_minuta == 18 || $cod_tipo_minuta == 21 || $cod_tipo_minuta == 22){
...
if ($cod_tipo_minuta == 8 || $cod_tipo_minuta == 9  || $cod_tipo_minuta == 10  || $cod_tipo_minuta == 14 || $cod_tipo_minuta == 55){
...
$hojaExcel.="<table width='100%'>";
  ...
  $hojaExcel.="<tr>";
      ...
      $instruccion7 ="SELECT DISTINCT calculo_requerimientos_inf.cod_ingrediente AS cod_ingrediente, ingrediente.nombre AS nom_ingrediente, 
                      calculo_requerimientos_inf.cod_categoria_ingrediente AS cod_categoria, categoria_ingrediente.nombre AS nom_categoria, 
                      ingrediente.unidad_base AS unidad_base 
                      FROM calculo_requerimientos_inf 
                      INNER JOIN ingrediente ON ingrediente.cod_ingrediente = calculo_requerimientos_inf.cod_ingrediente 
                      INNER JOIN categoria_ingrediente ON categoria_ingrediente.cod_categoria_ingrediente = calculo_requerimientos_inf.cod_categoria_ingrediente
                      WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_escuela = $cod_escuela
                        AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad AND calculo_requerimientos_inf.cod_tipo_minuta= $cod_tipo_minuta
                      ORDER BY calculo_requerimientos_inf.cod_categoria_ingrediente, calculo_requerimientos_inf.cod_ingrediente";
      ...
      for ($i=0; $i<$nfilas7; $i++){
        ...
        $sql9b = "SELECT SUM(calculo_redondeado_escuela_inf.cantidad_redondeada) AS sum_cantidad
                  FROM calculo_redondeado_escuela_inf 
                  WHERE calculo_redondeado_escuela_inf.cod_programacion = $cod_programacion AND calculo_redondeado_escuela_inf.cod_escuela = $cod_escuela 
                    AND calculo_redondeado_escuela_inf.cod_modalidad = $cod_modalidad AND calculo_redondeado_escuela_inf.cod_ingrediente = $cod_ingrediente
                    AND calculo_redondeado_escuela_inf.cod_tipo_minuta= $cod_tipo_minuta";
        ...
        $sql8a = "SELECT unidad_medida.nombre AS nombre
                  FROM calculo_redondeado_escuela_inf
                  LEFT JOIN unidad_medida ON unidad_medida.cod_unidad_medida = calculo_redondeado_escuela_inf.cod_unidad_medida 
                  WHERE calculo_redondeado_escuela_inf.cod_programacion = $cod_programacion AND calculo_redondeado_escuela_inf.cod_escuela = $cod_escuela 
                      AND calculo_redondeado_escuela_inf.cod_modalidad = $cod_modalidad  AND calculo_redondeado_escuela_inf.cod_ingrediente = $cod_ingrediente
                      AND calculo_redondeado_escuela_inf.cod_tipo_minuta= $cod_tipo_minuta";
        ...
        for ($h=0; $h<$nfilas6; $h++){             
         ...
         if($cod_menu <> ''){
          ...
          $instruccion_q ="SELECT SUM(calculo_requerimientos_inf.cantidad) AS suma  
                           FROM calculo_requerimientos_inf 
                           WHERE calculo_requerimientos_inf.cod_programacion = $cod_programacion AND calculo_requerimientos_inf.cod_escuela = $cod_escuela 
                             AND calculo_requerimientos_inf.cod_menu = $cod_menu AND calculo_requerimientos_inf.cod_modalidad = $cod_modalidad 
                             AND calculo_requerimientos_inf.cod_ingrediente = $cod_ingrediente AND calculo_requerimientos_inf.cod_tipo_minuta= $cod_tipo_minuta";
          ...
          $row_q = mysql_fetch_array($consulta_q);           
            ...
            $redondear2 = $row_r2['redondear2'];
              ...
              if($redondear2 == 1){
                 ...
                 $instruccion_und_2 ="SELECT calculo_redondeado_escuela_inf.cod_unidad_medida AS cod_unidad_medida, unidad_medida.valor_gr_cc AS valor_gr_cc
                                      FROM calculo_redondeado_escuela_inf
                                      INNER JOIN unidad_medida ON unidad_medida.cod_unidad_medida = calculo_redondeado_escuela_inf.cod_unidad_medida  
                                      WHERE calculo_redondeado_escuela_inf.cod_programacion = $cod_programacion AND calculo_redondeado_escuela_inf.cod_escuela = $cod_escuela 
                                        AND calculo_redondeado_escuela_inf.cod_modalidad = $cod_modalidad AND calculo_redondeado_escuela_inf.cod_ingrediente = $cod_ingrediente
                                        AND calculo_redondeado_escuela_inf.cod_tipo_minuta= $cod_tipo_minuta";
...
////BUSCAMOS LOS INGREDIENTES QUE MANEJAN INVENTARIO
  $instruccion7 ="SELECT DISTINCT calculo_redondeado_escuela_inf.cod_ingrediente AS cod_ingrediente, ingrediente.nombre AS nom_ingrediente,
                                  calculo_redondeado_escuela_inf.cod_categoria_ingrediente AS cod_categoria, categoria_ingrediente.nombre AS nom_categoria, 
                                  ingrediente.unidad_base AS unidad_base, calculo_redondeado_escuela_inf.cantidad_gr_cc AS cantidad_gr_cc,
                                  calculo_redondeado_escuela_inf.q_inventario_inicial_gr_cc AS q_inventario_inicial_gr_cc,
                                  calculo_redondeado_escuela_inf.q_final_gr_cc AS q_final_gr_cc,
                                  calculo_redondeado_escuela_inf.q_inventario_final_gr_cc AS q_inventario_final_gr_cc,
                                  calculo_redondeado_escuela_inf.cantidad_redondeada AS q_redondeada_final,
                                  calculo_redondeado_escuela_inf.cod_unidad_medida AS cod_unidad_medida_final,
                                  unidad_medida.nombre AS nom_unidad_medida    
                  FROM calculo_redondeado_escuela_inf 
                  INNER JOIN ingrediente ON ingrediente.cod_ingrediente = calculo_redondeado_escuela_inf.cod_ingrediente 
                  INNER JOIN categoria_ingrediente ON categoria_ingrediente.cod_categoria_ingrediente = calculo_redondeado_escuela_inf.cod_categoria_ingrediente
                  INNER JOIN unidad_medida ON unidad_medida.cod_unidad_medida = calculo_redondeado_escuela_inf.cod_unidad_medida
                  WHERE calculo_redondeado_escuela_inf.cod_programacion = $cod_programacion AND calculo_redondeado_escuela_inf.cod_escuela = $cod_escuela
                    AND calculo_redondeado_escuela_inf.cod_modalidad = $cod_modalidad AND calculo_redondeado_escuela_inf.cod_tipo_minuta= $cod_tipo_minuta
                    AND calculo_redondeado_escuela_inf.maneja_inventario = 1 
                  ORDER BY calculo_redondeado_escuela_inf.cod_categoria_ingrediente, calculo_redondeado_escuela_inf.cod_ingrediente";
