...
if($tipo_operacion == 2){
   ...
   $instruccion3 = "SELECT cod_plato, nombre, cod_grupo_alimento, panaderia FROM plato WHERE cod_plato = $codigo";
   $consulta3 = mysql_query ($instruccion3, $conexion);  
   $row3 = mysql_fetch_array ($consulta3);

   $cod_plato  = $row3['cod_plato'];  
   $nombre    = $row3['nombre']; 
   $cod_grupo_alimento = $row3['cod_grupo_alimento'];
   $panaderia = $row3['panaderia'];
...
<body>
<form action="vr_plato.php" method="post">
...
?> 
  ...
  <?php
    ...
    print ("<TD style='font-weight:bold; color: white'>Grupo</TD>");
    print ("<TD><img src='../imagenes/requerido.gif'><SELECT NAME='grupo'>");                
    ...
    $row = mysql_fetch_array ($consulta); 
      ...
      do{ 
        if($row['cod_grupo_alimento'] == $cod_grupo_alimento){
          print("<option value=".$row['cod_grupo_alimento']." Selected>[".$row['cod_grupo_alimento']."] - ".$row['nombre']."</option>");
         }else{ 
  ...
  <tr>
   <td style='font-weight:bold; color: white'>Panaderia</td>
   <td>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" <?php print("$checked");?> name="panaderia"> &nbsp;&nbsp;&nbsp;&nbsp; <span  style='font-weight:bold; color: white'>Se usa para el informe de Panaderia</span></td>   
  </tr>     
  <tr>
